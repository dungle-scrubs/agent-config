...
SUPPORTED: TypeScript, Python (ty/pyright), Rust, Swift`,
		...
		async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
			...
			const conn = await getOrCreateConnectionForFile(language, filePath);
			if (!conn) {
				return {
					details: {},
					content: [
			...
			if (!conn.capabilities.definitionProvider) {
				return {
					details: {},
					content: [{ type: "text", text: "Definition provider not supported by this language server" }],
				};
			}
...
- Finding callers of a function`,
		...
		async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
			...
			try {
				...
				return {
					details: {},
					content: [{ type: "text", text: `Found ${locations.length} reference(s):\n${formatLocations(locations)}` }],
				};
			} catch (error) {
				return {
					details: {},
		...
		async execute(_toolCallId, params, _signal, _onUpdate, _ctx) {
			...
			try {
				...
				return {
					details: {},
					content: [
						{ type: "text", text: `Found ${symbols.length} symbol(s) in ${conn.rootPath}:\n${formatSymbols(symbols)}` },
					],
				};
			} catch (error) {
				return {
