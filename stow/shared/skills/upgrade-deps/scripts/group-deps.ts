...
const SAME_VERSION_GROUPS = new Set([
  "storybook",
  "trpc",
  "tanstack",
  "prisma",
...
function groupDependencies(input: AnalyzeOutput): GroupedOutput {
  ...
  const autoUpdateable =
    groups.filter((g) => !g.requiresUserApproval).reduce((sum, g) => sum + g.packages.length, 0) +
    ungroupedSafe.length;

  const requiresApproval = groups
    .filter((g) => g.requiresUserApproval)
    .reduce((sum, g) => sum + g.packages.length, 0);
...
async function main(): Promise<void> {
  ...
  if (stdinData.trim()) {
    ...
    if (!existsSync(analyzeScript)) {
      console.error(
        JSON.stringify({ error: "analyze-deps.ts not found at " + analyzeScript })
      );
      process.exit(1);
    }
    ...
    try {
      ...
      console.error(
        JSON.stringify({ error: "Failed to run analyze-deps.ts", details: err.stderr })
      );
      process.exit(1);
    }
  }
